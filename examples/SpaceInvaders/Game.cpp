...
void Game::showScore(bool on){
  ...
  m_tft->setCursor(GAMEX + SCREENSX/2+10, BOMBEND+6);
  m_tft->print("Score:");	m_tft->print(this->m_score);
	this->m_tft->drawLine(GAMEX, BOMBEND, GAMEX+SCREENSX, BOMBEND, CYAN);
  this->m_tft->drawLine(GAMEX, BOMBEND+1, GAMEX+SCREENSX, BOMBEND+1, CYAN);
}
...
void Game::init(){
  ...
  m_tft->fillScreen(CLEAR);
	...
	showScore(true);
  this->m_tft->drawLine(GAMEX-10, 20, GAMEX-10, LCDHEIGHT-30, CYAN);
  this->m_tft->drawLine(GAMEX-11, 20, GAMEX-11, LCDHEIGHT-30, CYAN);
	//8, 2
}
